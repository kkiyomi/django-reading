...
class Novel(models.Model):
    ...
    image = models.ImageField(default='default.jpg', upload_to='novel_pics', max_length=500)
...
class NovelQu(models.Model):
    ...
    novel = models.OneToOneField(Novel, on_delete=models.SET_NULL, blank=True, null=True)
...
class ChapterQUepubs(models.Model):
    ...
    novel_qu = models.ForeignKey(NovelQu, on_delete=models.SET_NULL, blank=True, null=True)

    def save(self, *args, **kwargs):
        if self.novel_qu is None:
            self.novel_qu, _ = NovelQu.objects.get_or_create(novel_title=self.novel_title)
